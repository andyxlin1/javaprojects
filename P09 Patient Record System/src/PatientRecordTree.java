...
public class PatientRecordTree {
  ...
  public static String toStringHelper(PatientRecordNode current) {
    ...
    returnString += current.getPatientRecord().toString() + "\n";
    if (current.getRightChild() != null) {
  ...
  public String toString() {
    return toStringHelper(this.root).trim();
  }
  ...
  private PatientRecord lookupHelper(PatientRecord findRecord, PatientRecordNode current) {
    if (current == null) {
      throw new NoSuchElementException("No patient record in BST matches provided date.");
    }
  ...
  public static int heightHelper(PatientRecordNode current) {
    ...
    int leftTraversal = heightHelper(current.getLeftChild());
    int rightTraversal = heightHelper(current.getRightChild());
  ...
  public PatientRecord getRecordOfYoungestPatient() {
    if (isEmpty())
      return null;
    PatientRecordNode returnChild = this.root;
    while (returnChild.getRightChild() != null) {
      returnChild = returnChild.getRightChild();
    }
    return returnChild.getPatientRecord();
  }
  ...
  public PatientRecord getRecordOfOldestPatient() {
    if (isEmpty())
      return null;
    PatientRecordNode returnChild = this.root;
    while (returnChild.getLeftChild() != null) {
      returnChild = returnChild.getLeftChild();
    }
    return returnChild.getPatientRecord();
  }
}
